...
class StageStatus:
    ...
    def __init__(self, pad_result: tuple = None, width=0, height=0):
        self.width = width
        self.height = height
        self.dy = self.edy = self.dx = self.edx = self.y = self.ey = self.x = self.ex = (
            self.tmpw
        ) = self.tmph = []
...
class MTCNN:
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        points[5:10, :] = (
            np.tile(h, (5, 1)) * points[5:10, :] + np.tile(total_boxes[:, 1], (5, 1)) - 1
        )
...
def mtcnn_caffe_detect(
    ...
    """Detect faces with MTCNN using Caffe weights
